...
These are stored securely (the password goes to the system keychain) and passed to the bundled MCP server, which Cowork starts automatically — there's nothing to run yourself.

Requires `uv` (Python 3.12+) on the machine running Cowork; the plugin invokes `uv run freshrss-mcp` to start the server.
...
- **`freshrss-people`** — `/freshrss-people [timeframe]` summarizes recent posts from your FreshRSS "People" category, person by person — a catch-up with people you follow rather than a link list.
